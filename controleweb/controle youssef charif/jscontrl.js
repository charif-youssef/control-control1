...
function valide() {
    ...
    if (!email_check) {
        Error_conatiner.innerHTML = '<span>il ya un erore</span>';
        animateDiv();
    }
    if (!pass_check) {
        Error_conatiner.innerHTML += '<span>Passowrd error</span>';
        animateDiv();
    }
    if (!check) {
        Error_conatiner.innerHTML += '<span>Fillier error</span>';
        animateDiv();
    }
    if (!vile__) {
        Error_conatiner.innerHTML += '<span>il faut choisie ville</span>';
        animateDiv();
    }
    if (!check && !email_check && !pass_check && !vile__) {
        Error_conatiner.innerHTML = '<span> vide </span>';
        animateDiv();
